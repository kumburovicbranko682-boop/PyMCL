...
class BackendAPI:
    ...
    # ---- 皮肤更换（微软正版 / authlib 皮肤站），对齐 app.backend
    ...
    def upload_skin(self, account_name: str, file_path: str,
        ...
        return self.start_task(
            "皮肤上传", self._upload_skin_impl, account_name, file_path, variant)
    ...
    def reset_skin(self, account_name: str) -> str:
        return self.start_task("皮肤重置", self._reset_skin_impl, account_name)
    ...
    def set_cape(self, account_name: str, cape_id: str = "") -> str:
        return self.start_task("皮肤披风", self._set_cape_impl, account_name, cape_id)
    ...
    def search_modpacks(self, query: str, source: str, extra: dict | None = None) -> list[dict]:
        ...
        if not hits:
            try:
                if src == "curseforge":
                    hits = modpack_mod.search_cf_modpacks(dm, q, limit=30, api_key=key,
                                                          offset=offset)
                else:
                    hits = modpack_mod.modrinth_search(dm, q, limit=30, offset=offset)
            except Exception:
                hits = []
        else:
            hits = sorted(
    ...
    def get_java_list(self, scan_system: bool = False) -> list[dict]:
        if scan_system:
            ...
            javas = java_mod.list_installed_javas() + java_mod.custom_javas()
        rows = []
        for j in javas:
            rows.append({
    ...
    def _install_modpack_impl(self, progress, log, name, source, extra=None):
        ...
        if manual:
            log("以下 Mod 作者禁止第三方下载（CurseForge 403），请在浏览器打开链接手动下载，放入实例 mods 文件夹：")
            for m in manual:
                log(f"  · {m.get('project') or m.get('filename')}: {m.get('url')}")
            log("清单已写入 mods/需要手动下载的Mod.txt，放好文件后即可启动。")
        ...
        if manual:
            return f"整合包安装完成，但 {len(manual)} 个 Mod 需手动下载（见提示）"
