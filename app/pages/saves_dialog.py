...
class SavesDialog(MessageBoxBase):
    ...
    def _delete(self):
        ...
        if self.kind.currentText() == tr("备份"):
            if MessageBox(tr("删除备份"), tr("确定删除备份「{0}」？").format(name), self).exec():
                ...
                self.reload()
            return
        box = MessageBox(tr("删除存档"), tr("确定删除「{0}」？").format(name), self)
        if box.exec():
    ...
    def _backup(self):
        ...
        try:
            ...
            return
        MessageBox(tr("已开始备份"),
                   tr("「{0}」正在打包，进度见「下载任务」。").format(name), self).exec()
    ...
    def _restore(self):
        ...
        try:
            ...
            return
        MessageBox(tr("还原完成"),
                   tr("已还原为存档「{0}」。").format(out.get("name")), self).exec()
    ...
    def _datapack(self):
        ...
        if not packs:
            MessageBox(tr("没有数据包"), tr("先到「下载 → 数据包」安装。"), self).exec()
            return
        ...
        if not dlg.exec():
            return
        try:
            self.backend.install_datapack_into_save(
                self.instance, box.currentText(), name, self.version)
        except Exception as e:
            MessageBox(tr("安装失败"), str(e), self).exec()
            return
        MessageBox(tr("安装完成"),
                   tr("数据包已装进存档「{0}」。").format(name), self).exec()