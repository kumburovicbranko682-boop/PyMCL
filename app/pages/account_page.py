...
class AccountPage(QWidget):
    ...
    def reload(self):
        ...
        if not rows:
            # 这个列表收录所有类型（含离线），文案别只提两种；顺便指路。
            self.list_box.addWidget(CaptionLabel(
                tr("还没有账号——在下面登录微软 / 皮肤站，或直接保存一个离线账号")))
        for row in rows:
            card = QWidget()
        ...
        # 没有账号就老实写「未登录」，别顶着一个不存在的「Steve」假装登录过
    ...
    def _ms(self):
        ...
        self._login_dlg = None
        # 关掉设备码框就是放弃登录：与启动页同款处理——不取消的话
        # 后台轮询会一直问微软要令牌，成功了还会静默加账号。
        if not accepted and self._login_task:
            try:
                self.backend.cancel_task(self._login_task)
            except Exception:
                pass
            self._login_task = None
        self.reload()
