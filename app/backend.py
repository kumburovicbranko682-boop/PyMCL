...
class BackendAPI(QObject):
    ...
    def install_i18n_mod(self, instance: str = "default", version: str = "") -> str:
        ...
        if not mods_mod.detect_loader(inst):
            raise ValueError(tr("需要先安装模组加载器（Fabric / Forge / Quilt / NeoForge）"))
        mods_dir = self._mods_folder(inst, version) if version else None
        existing = mods_mod.i18n_mod_installed(inst, mods_dir)
        if existing:
            raise ValueError(tr("已安装汉化模组: {name}").format(name=existing))
    ...
    def duplicate_instance(self, name: str, new_name: str = "") -> str:
        ...
        return self.start_task(tr("复制实例") + f" {name}",
                               self._duplicate_instance_impl, name, new_name)

    def _duplicate_instance_impl(self, progress, log, name, new_name):
        from mclauncher import instances as inst_mod
        log(tr("正在复制实例 {name}…").format(name=name))
        final = inst_mod.duplicate_instance(
            name, new_name,
            on_progress=lambda done, total: progress(done, total, tr("复制文件")))
        self._emit_ui_changed()
        log(tr("已复制为新实例：{name}").format(name=final))
        return final
    ...
    def update_modpack(self, instance: str, version_id: str = "") -> str:
        return self.start_task(tr("更新整合包") + f" {instance}",
    ...
    def set_cape(self, account_name: str, cape_id: str = "") -> str:
        return self.start_task(tr("更换披风"), self._set_cape_impl,
                               account_name, cape_id)
    ...
    def ping_listed_server(self, instance: str, index: int) -> dict:
        ...
        if not entry:
            return {"online": False, "error": tr("服务器不存在"), "index": index}
