...
class BackendAPI(QObject):
    ...
    def export_launch_script(self, instance: str, version: str, dest: str = "") -> str:
        return self.start_task(tr("导出启动脚本") + f" {version}", self._export_bat_impl, instance, version, dest)
    ...
    def backup_save(self, instance: str, name: str, version: str = "") -> str:
        return self.start_task(tr("备份存档") + f" {name}", self._backup_save_impl, instance, name, version)
    ...
    def download_java(self, major: str, vendor: str = "adoptium") -> str:
        ...
        if vendor == "adoptium":
            return self.start_task(tr("下载 Java") + f" {major}", self._download_java_impl, major)
    ...
    def _launch_into_server(self, url: str, already_msg: str):
        ...
        if not ids:
            raise LaunchError(tr("请先到「下载 → 原版游戏」安装一个版本。"))
    ...
    def repair_version(self, instance: str, version: str) -> str:
        return self.start_task(tr("修复") + f" {version}", self._repair_impl, instance, version)
    ...
    def start_mod_updates(self, instance: str) -> str:
        return self.start_task(tr("检查模组更新") + f" {instance}", self._mod_update_impl, instance)
    ...
    def _launch_game_impl(self, progress, log, instance, version, account,
        ...
        if not version:
            raise LaunchError(tr("请先选择版本（到「下载 → 原版游戏」安装）"))
        # 多开检查
